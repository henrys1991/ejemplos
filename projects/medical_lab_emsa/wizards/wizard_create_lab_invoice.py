...
class create_test_invoice(osv.osv_memory):
    _name='medical.order.lab.invoice'

    def create_lab_invoice(self, cr, uid, ids, context={}):

        invoice_obj = self.pool.get('account.invoice')
        test_request_obj = self.pool.get('medical.order.lab')
  ...
  ####################recorrido para los productos#########################3
        
            for test_id in tests:
                #for test in test_request_obj.browse(cr, uid, test_id): #                 
                test = test_request_obj.browse(cr, uid, test_id)
                logging.debug('test.test_type_lab = %s', test.test_type_lab)
                for t in test.test_type_lab: #                
                    #logging.debug('test.test_type_lab = %s; test.test_type_lab.product_id = %s; test.test_type_lab.product_id.id = %s', test.test_type_lab, test.test_type_lab.product_id, test.test_type_lab.product_id.id)
                    logging.debug('t.product_id = %s; t.product_id.id = %s', t.product_id, t.product_id.id)
    
                    if prods_data.has_key(t.product_id.id):
                        #logging.debug('prods_data = %s; test.test_type_lab.product_id.id = %s', prods_data, test.test_type_lab.product_id.id)
                        logging.debug('prods_data = %s; t.product_id.id = %s', prods_data, t.product_id.id)
                        prods_data[t.product_id.id]['quantity'] += 1
                    else:
                        logging.debug('t.product_id.id = %s', t.product_id.id)
                        a = t.product_id.product_tmpl_id.property_account_income.id
                        if not a:
                            a = t.product_id.categ_id.property_account_income_categ.id
                            prods_data[t.product_id.id] = {'product_id':t.product_id.id,
                                                           'name':t.product_id.name,
                                                           'quantity':1,
                                                           'account_id':a,
                                                           'price_unit':t.product_id.list_price}
                            logging.debug('prods_data[t.product_id.id] = %s', prods_data[t.product_id.id])
