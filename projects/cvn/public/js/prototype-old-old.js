...
/*
 Mejoras a las funciones bind y bindasEventListener para corregir el problema del $A()
*/
...
Object.extend(String.prototype, {
  ...
  inspect: function() {
	...
	// Addon por Rafael Nevarez // Valida si un string es un email Valido
...
Ajax.Request.prototype = Object.extend(new Ajax.Base(), {
  ...
  /* Permite cancelar o detener un request
   * Giannella Paredes
   */
  stopRequest: function(){
	  try{this.transport.abort();} catch(e){};
  }
});
...
/*
toggleEliminados = function(){
	// Acceso a si_common.css, cambiamos el estilo directamente en la medula.
	document.styleSheets[0].cssRules[0].styleSheet.cssRules[2].style.display = (document.styleSheets[0].cssRules[0].styleSheet.cssRules[2].style.display == '' ? 'none' : '');
}

showEliminados = function(){
	document.styleSheets[0].cssRules[0].styleSheet.cssRules[2].style.display = '';
}

hideEliminados = function(){
	document.styleSheets[0].cssRules[0].styleSheet.cssRules[2].style.display = 'none';
}
*/
...
Element.Methods = {
  ...
  getChildrenByClassName: function(element, className){
   ...
   return elements;
	...
	swap: function(element, n2) {
		var p = n2.parentNode;
		var s = n2.nextSibling;
		element.parentNode.replaceChild(n2, element);
		p.insertBefore(element,s);
	},
	...
	centerInScreen: function(element, options){
		...
		var styleAttrs = $H({	left: L + 'px', top: T + 'px', position: 'absolute' });
...
/*
		if ( options.modal ){			
			...
			Element.setStyle(modal, $H({ width: window.getWidth() + 'px', height: window.getHeight() + 'px'}) );
...
*/
		...
		var styleAttrs = $H({	top: T + 'px', position: 'absolute', zIndex: '100' });
		...
		// Tapa toda la ventana padre.

		
		if (options.hideOnClick){
			Event.observe(document, 'click', function hideContext(evt){																														
				if ( Event.element(evt) != element ){
					Event.stopObserving(document, 'click', hideContext);					
					Element.hide(element);
				}					
			}, true);		
		}
...
var Form = {
  ...
  /* Por Rafael Nevarez 
   * Serializa los elementos del form, pero escapeados.
  */
  serializeEscaped: function(form) {
  alert('serializeEscaped-Form');
		var elements = Form.getElements($(form));
    var queryComponents = new Array();
  ...
  /* Por Rafael Nevarez ------------------------------
     Chequea que campos dentro de tag span class="obligatorio" esten llenos.
	 si encuentra error inserta textoAdvertencia despues del campo
  */
  /*chequearCamposObligatorios: function (form, textoAdvertencia){
		var formObligatorios = document.getElementsByClassName('obligatorio', form);
		var validForm = true;
		for (var i=0; i<formObligatorios.length; i++)
		{
			var inputOriginal = formObligatorios[i].firstChild;
			if (formObligatorios[i].firstChild.value == ''){
				formObligatorios[i].innerHTML = "";
				formObligatorios[i].appendChild(inputOriginal);
				//new Insertion.Bottom(formObligatorios[i], inputOriginal);
				new Insertion.Bottom(formObligatorios[i], textoAdvertencia);
				validForm = false;			
			}
			else{
				formObligatorios[i].innerHTML = "";
  ...
  chequearCamposObligatorios: function (form, textoAdvertencia){
		try{
			Form.eliminarMensajesAdvertencia(form, 'advertencia'); 
		} catch(e){};
  ...
  /* Por Rafael Nevarez ------------------------------
     Nuevas Funciones para validar Forms. Chequea Campos "obligatorio", "numerico"
  */
  
  isValid: function(form){
  		var elements = Form.getElements(form);
  		var invalidField = elements.find(function(e){
			return ( (Element.hasClassName(e.parentNode, 'obligatorio') && e.value == '') || (Element.hasClassName(e.parentNode, 'numerico') && isNaN(e.value))  );
  		});
  ...
  showErrors: function(form, options){
    	...
    	var elements = Form.getElements(form);
	  	elements.each(function(e){
			...
			if ( Element.hasClassName(e.parentNode, 'numerico') && isNaN(e.value) ){
  ...
  clearErrors: function(form){
		var elements = document.getElementsByClassName('_error_', form);
		elements.each(function(e){
			Element.remove(e);
		});
  },
  
  dateDiff: function(elementFechaInicio, elementFechaFinal){
		if (arguments.length != 2) return false;
		var dateInicial = Form.Element.isDate(elementFechaInicio);
		var dateFinal = Form.Element.isDate(elementFechaFinal);
		if ( dateFinal && dateInicial ){
			return(Math.ceil(dateFinal - dateInicial));
		}else{
			return false;
		}
  }
...
Form.Element = {
  ...
  alert('serializeEscaped-Element');
    element = $(element);
    var method = element.tagName.toLowerCase();
    var parameter = Form.Element.Serializers[method](element);
...
/* Editado por Andr�s Sussmann -- valida la fecha de una vez formato "yyyy-mm-dd" */
String.toDate = function(date_string){
	if (typeof date_string != 'string') return false;
	regexp = /^((19|20)\d\d)-(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[01])$/;
	array_fecha = regexp.exec(date_string);
	if ( array_fecha != null){
		//alert('anio: "'+ array_fecha[1] + '"\nmes: "'+ array_fecha[3] + '"\ndia: "'+ array_fecha[4] + '"');
		var anio = parseInt(array_fecha[1] - 0);
		var mes = parseInt(array_fecha[3] - 0);
		var dia = parseInt(array_fecha[4] - 0);
		if (dia == 31 && (mes == 4 || mes == 6 || mes == 9 || mes == 11)) {
		  return false; // 31st of a month with 30 days
		} else if (dia >= 30 && mes == 2) {
		  return false; // February 30th || 31st
		} else if (mes == 2 && dia == 29 && !(anio%4 == 0 && (anio%100 != 0 || anio%400 == 0))) {
		  return false; // February 29th outside a leap year
		} else {
			var dateAsDateObject = new Date;
			dateAsDateObject.setYear(anio);
			dateAsDateObject.setMonth(mes-1);
			dateAsDateObject.setDate(dia);
			dateAsDateObject.setHours(0);
			dateAsDateObject.setMinutes(0);
			dateAsDateObject.setSeconds(0);
  ...
  selectOne: function(element) {
    ...
    if (index >= 0) {
      opt = element.options[index];
			value = opt.value;      //value = opt.value || opt.text;
    }
...
window.showDialog = function(parent, options){
	var options = Object.extend({
		id: 'modalDialog',
		className: 'popupForm',
		containerClassName: '',
	...
	if ( options.url ){
		new Ajax.Updater(container, options.url, {
			 ...
			 onComplete: function() {
				...
				Element.showContext(modalDialog, {top: posTop, left:posLeft, hideOnClick: false, hideOnEsc: options.hideOnEsc, modal: true });
...
/*--------------------------------------------------------------------------*/
/*Date.isDate = function(fecha){
	if (!fecha) return false;
	if (typeof fecha != 'string') return false;
	var date = fecha.split('-');
	var anio = parseInt(date[0] - 0);
	var mes = parseInt(date[1] - 0);
	var dia = parseInt(date[2] - 0);
	if ( anio && mes && dia ){
		//var dateAsDateObject = new Date;
		this.setYear(anio);
		this.setMonth(mes-1);
...
Date.dateDiff = function(fechaInicio, fechaFinal){
	if (arguments.length != 2) return false;
	var dateInicial = Date.isDate(fechaInicio);
	var dateFinal = Date.isDate(fechaFinal);
	return(Math.ceil(dateFinal - dateInicial));
}*/