...
*/
	...
	$query = "SELECT * FROM persona WHERE estado='_PENDIENTE'"; //se consultan todas las personas q no se han procesado
	...
	while($filas=$conexion->fetch_array($enlace)){		
		...
		if($status){			
			...
			$sqlUpdate="UPDATE persona SET xml=\"$xmGuardar\", estado='_PROCESADO' WHERE id='$id'";
		    $conexion->query($sqlUpdate);
		} 
		else{
			$flagError=true;			
			$sqlUpdate="UPDATE persona SET estado='_ERROR' WHERE id='$id'";
			$conexion->query($sqlUpdate);
...
function enviarCorreo($msj){
	...
	$mensaje .= 'Durante la ejecucion del script de procesamiento de CV se produjeron algunos errores. A continuacion se detalla los errores:<br/><br/>';
	$mensaje .= $msj;
	$mensaje .= '<br/> Saludos';	
